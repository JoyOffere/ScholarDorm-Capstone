...
import { BellIcon, SearchIcon, FilterIcon, PlusIcon, SendIcon, EyeIcon, TrashIcon, CheckCircleIcon, AlertCircleIcon, InfoIcon, XCircleIcon, UsersIcon, CalendarIcon, RefreshCwIcon, ClockIcon, XIcon } from 'lucide-react';
...
export const AdminNotifications: React.FC = () => {
  ...
  const handleSendNotification = async (notificationId: string) => {
    try {
      ...
      if (!user) {
        throw new Error('You must be logged in to send a notification');
      }
      // Get notification details
      const {
        data: notification,
        error: fetchError
      } = await supabase.from('notifications').select('target_audience').eq('id', notificationId).single();
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('notifications').delete().eq('id', notificationToDelete);
      if (error) throw error;
      // Create audit log
      if (user) {
  ...
  return <DashboardLayout title="Notifications Management" role="admin">
      {/* Error Message */}
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="relative flex-1">
            ...
            fetchNotifications();
              ...
              <input type="text" className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" placeholder="Search notifications..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </form>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            {/* Status Filter */}
            <div className="relative inline-block text-left">
              <select className="block appearance-none bg-white border border-gray-300 rounded-lg py-2 pl-3 pr-8 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" value={statusFilter} onChange={e => setStatusFilter(e.target.value as any)}>
            ...
            {/* Type Filter */}
            <div className="relative inline-block text-left">
              <select className="block appearance-none bg-white border border-gray-300 rounded-lg py-2 pl-3 pr-8 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" value={typeFilter} onChange={e => setTypeFilter(e.target.value as any)}>
                <option value="all">All Types</option>
                <option value="system">System</option>
                <option value="announcement">Announcement</option>
                <option value="streak_reminder">Streak Reminder</option>
                <option value="badge_earned">Badge Earned</option>
                <option value="course_recommendation">
            ...
            {/* Audience Filter */}
            <div className="relative inline-block text-left">
              <select className="block appearance-none bg-white border border-gray-300 rounded-lg py-2 pl-3 pr-8 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" value={audienceFilter} onChange={e => setAudienceFilter(e.target.value as any)}>
            ...
            {/* Reset Filters */}
            {(searchTerm || statusFilter !== 'all' || typeFilter !== 'all' || audienceFilter !== 'all') && <button onClick={() => {
            setSearchTerm('');
            setStatusFilter('all');
            setTypeFilter('all');
            ...
            {/* Refresh */}
            <button onClick={fetchNotifications} className="inline-flex items-center px-3 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500">
              <RefreshCwIcon size={16} className="mr-2" />
              Refresh
            </button>
            {/* Create Notification */}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                  ...
                  <td colSpan={6} className="px-6 py-4 text-center">
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-600">
                        {searchTerm || statusFilter !== 'all' || typeFilter !== 'all' || audienceFilter !== 'all' ? 'Try adjusting your search or filter criteria.' : 'Create your first notification to get started.'}
                      </p>
                    </div>
                  </td>
                </tr> : notifications.map(notification => <tr key={notification.id} className="hover:bg-gray-50">
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          ...
                          <div className="flex items-center mt-2 space-x-2">
                            ...
                            {notification.recipient_count !== undefined && notification.recipient_count > 0 && <span className="text-xs text-gray-500">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      {notification.scheduled_for && !notification.is_sent && <div className="text-xs text-blue-600 mt-1 flex items-center">
                          <ClockIcon size={12} className="mr-1" />
                          Scheduled: {formatDate(notification.scheduled_for)}
                        </div>}
                      {notification.sent_at && <div className="text-xs text-green-600 mt-1 flex items-center">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        {!notification.is_sent && <button onClick={() => handleSendNotification(notification.id)} className="text-green-600 hover:text-green-900" title="Send now">
                            <SendIcon size={16} />
                          </button>}
                        <button onClick={() => handleDeleteClick(notification.id)} className="text-red-600 hover:text-red-900" title="Delete">
                          <TrashIcon size={16} />
                        </button>
                      </div>
                    </td>
        ...
        {/* Pagination */}
        {!loading && notifications.length > 0 && totalPages > 1 && <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              ...
              <div>
                ...
                return <button key={i} onClick={() => setPage(pageNum)} className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium
                  ...
                  <button onClick={() => setPage(Math.min(totalPages, page + 1))} disabled={page >= totalPages} className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50">
                    ...
                    <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                      <path fillRule="evenodd" d="M7.293 14.707a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" />
                    </svg>
                  </button>
                </nav>
              </div>
          ...
          <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    ...
                    <div className="mt-4 space-y-4">
                      <div>
                        ...
                        <input type="text" value={newNotification.title} onChange={e => setNewNotification({
                      ...newNotification,
                      title: e.target.value
                    })} className="w-full border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="Notification title..." />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          Message
                        </label>
                        <textarea value={newNotification.message} onChange={e => setNewNotification({
                      ...newNotification,
                      message: e.target.value
                    })} rows={3} className="w-full border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="Notification message..." />
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <select value={newNotification.type} onChange={e => setNewNotification({
                        ...newNotification,
                        ...
                        <div>
                          ...
                          <select value={newNotification.urgency} onChange={e => setNewNotification({
                        ...newNotification,
                      ...
                      <div>
                        ...
                        <select value={newNotification.target_audience} onChange={e => setNewNotification({
                      ...newNotification,
                      ...
                      <div>
                        ...
                        <input type="text" value={newNotification.action_url} onChange={e => setNewNotification({
                      ...newNotification,
                      action_url: e.target.value
                    })} className="w-full border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="https://example.com/action" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          Schedule For (Optional)
                        </label>
                        <input type="datetime-local" value={newNotification.scheduled_for} onChange={e => setNewNotification({
                      ...newNotification,
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button onClick={handleCreateNotification} className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-purple-600 text-base font-medium text-white hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 sm:ml-3 sm:w-auto sm:text-sm">
                  Create Notification
                </button>
                <button onClick={() => setShowCreateModal(false)} className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                  Cancel
                </button>
              </div>
            </div>
